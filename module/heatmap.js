...
import { degToRad, mapPointFromLngLat, mat4create, mat4fromTranslationScale, mat4mul, vec2add, vec2floor, vec2max, vec2min, vec2mul, vec2normalize, vec2sub, } from '@trufi/utils';
import { startAnimation, updateAnimation } from './animation';
...
export class Heatmap {
    constructor(map, container, options) {
        ...
        this.update = () => {
            ...
            mat4mul(tempMatrix, mapMatrix, this.matrix);
    ...
    setData(points) {
        ...
        this.findViewportMinMaxTemp();
        mat4fromTranslationScale(this.matrix, [grid.minX, grid.minY, 0], [grid.stepX, grid.stepY, 1]);
        const array = [];
        let i = 0;
    ...
    findViewportMinMaxTemp() {
        ...
        temps.sort((a, b) => a - b);
        startAnimation(this.minValue, temps[Math.floor(temps.length * this.options.gridMinPercentile)], adaptivePalleteDuration);
        startAnimation(this.maxValue, temps[Math.min(Math.floor(temps.length * this.options.gridMaxPercentile), temps.length - 1)], adaptivePalleteDuration);
    }
}
